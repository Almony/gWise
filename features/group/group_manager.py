...
logger = CustomLogger("GroupManager")
...
class GroupManager:
    ...
    async def add_group(self, group_id: int, title: str, type_: str, owner_id: int):
        groups = self.mongo.get_collection(MongoCollections.GROUPS)
        existing = await groups.find_one({"group_id": group_id})
        if existing:
            logger.debug(f"Группа уже существует: {group_id}")
            return
        doc = GroupSchema(
            group_id=group_id,
            title=title,
        ...
        logger.info(f"Добавлена новая группа: {title} ({group_id})")

    async def save_post(self, data: GroupPostSchema):
        posts = self.mongo.get_collection(MongoCollections.GROUP_POSTS)
        await posts.insert_one(data.dict())
        logger.debug(f"Пост {data.post_id} в группе {data.group_id} сохранён")

    async def add_member(self, data: GroupMemberSchema):
        members = self.mongo.get_collection(MongoCollections.GROUP_MEMBERS)
        await members.update_one(
            {"group_id": data.group_id, "user_id": data.user_id},
            {"$set": data.dict()},
            upsert=True
        )
        logger.debug(f"Участник {data.user_id} добавлен/обновлён в {data.group_id}")
    ...
    async def log_member_activity(self, group_id: int, user_id: int, field: str):
        ...
        logger.debug(f"{field} зафиксировано у {user_id} в {group_id}")
